...
let ejs = fs.readFileSync('views/pages/tasks.ejs', 'utf-8');
...
// Insert the modal before the end of the body/main container
if (!ejs.includes('id="editTaskModal"')) {
  ejs = ejs.replace('</main>', editModalHTML + '\n</main>');
}

// Add allTasks definition inside the scripts block
if (!ejs.includes('const allTasks')) {
  ejs = ejs.replace('<script>', '<script>\n  const allTasks = <%- JSON.stringify(tasks) %>;');
}
...
fs.writeFileSync('views/pages/tasks.ejs', ejs);
console.log('Successfully injected Edit Task modal into tasks.ejs');
