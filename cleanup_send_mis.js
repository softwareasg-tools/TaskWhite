...
async function cleanupSendMIS() {
  try {
    ...
    typesSnap.docs.forEach(doc => {
      const data = doc.data();
      typeMap[doc.id] = data;
      if (data.name && data.name.toLowerCase().includes('send mis')) {
        targetTypeIds.add(doc.id);
        console.log(`Found Target Task Type: "${data.name}" (ID: ${doc.id})`);
      }
    });
    ...
    for (const doc of tasksSnap.docs) {
      ...
      const isTarget = targetTypeIds.has(data.task_type_id) || 
                       typeName.includes('send mis') ||
                       (data.title && data.title.toLowerCase().includes('send mis')) ||
                       (data.name && data.name.toLowerCase().includes('send mis'));
    ...
    for (const doc of templatesSnap.docs) {
      ...
      if (targetTypeIds.has(data.task_type_id) || typeName.includes('send mis')) {
        batch.delete(doc.ref);
    ...
    console.log(`SUCCESS! Wiped ${deletedTaskCount} "Send MIS" tasks, ${deletedTemplateCount} templates, and ${targetTypeIds.size} task types from backend.`);
    process.exit(0);
  } catch (err) {
